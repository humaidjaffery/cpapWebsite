...
export interface OpenAiSnippetResult {
...
export async function generatePersonalizedSnippet(params: {
  apiKey: string;
  model: string;
  survey: SanitizedSurvey;
}): Promise<OpenAiSnippetResult> {
  ...
  const outputJson = JSON.parse(outputText) as { personalized_snippet?: unknown };
