...
import { MaskGallery, MaskIndex, MaskPriceIndex, MaskPrices, MaskProfile } from './mask-data';
...
export class MaskDataService {
  ...
  getGallery(slug: string): Observable<MaskGallery | null> {
    return this.http
      .get<MaskGallery>(`/data/mask-galleries/${encodeURIComponent(slug)}.json`)
  ...
  getPriceIndex(): Observable<MaskPriceIndex | null> {
    return this.http
      .get<MaskPriceIndex>('/data/prices/index.json')
      .pipe(catchError(() => of(null)));
  }
}
