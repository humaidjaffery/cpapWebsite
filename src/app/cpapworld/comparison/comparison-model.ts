...
export interface MaskComparisonModel {
  ...
  defaultCriteria: AlignedComparisonFinding[];
  availableCriteria: AlignedComparisonFinding[];
...
export function buildMaskComparison(
  ...
  return {
    ...
    criteria,
    defaultCriteria: DEFAULT_COMPARISON_CRITERIA.flatMap((id) => {
      const row = criteria.find((candidate) => candidate.id === id);
      return row ? [row] : [];
    }),
...
function eligibilityErrors(
  ...
  return profile.coverage.processedReviews < 50
    ? [
        {
          side,
...
function classifyFindings(
  ...
  if (leftFinding.reviewCount < 10 || rightFinding.reviewCount < 10) {
    return [
      {
        finding: leftFinding,
        state: 'neutral',
        labels: [leftFinding.reviewCount < 10 ? 'Limited evidence' : 'Neutral']
      },
      {
        finding: rightFinding,
        state: 'neutral',
        labels: [rightFinding.reviewCount < 10 ? 'Limited evidence' : 'Neutral']
      }
    ];
  }
...
function classifyBodySides(
  ...
  if (left.relevantReviews < 10 || right.relevantReviews < 10) {
    left.labels = [left.relevantReviews < 10 ? 'Limited evidence' : 'Neutral'];
    right.labels = [right.relevantReviews < 10 ? 'Limited evidence' : 'Neutral'];
    return;
  }
...
function notableComponents(parts: PartFinding[]): NotableComponent[] {
  const qualified = parts.filter(
    (part) => part.reviewCount >= 10 && part.evidenceStrength !== 'limited'
  );
